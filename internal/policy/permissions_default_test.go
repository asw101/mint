...
func TestGrantWithoutPermissionsCoversAnUnrestrictedRequest(t *testing.T) {
	...
	g := Grant{Repos: []string{"gadget", "widget"}}

	if !g.Covers(Scope{Repos: []string{"gadget"}}) {
		t.Error("want an unrestricted request to be covered")
	}
	if !g.Covers(Scope{Repos: []string{"gadget"}, Permissions: map[string]string{"contents": "write"}}) {
